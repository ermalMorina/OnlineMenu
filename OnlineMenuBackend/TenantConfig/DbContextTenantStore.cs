        ...
        if (tenant is null)
        {
            var entity = await _context.Tenants
                .FirstOrDefaultAsync(q => q.Identifier == identifier)
                    ?? throw new ArgumentNullException($"identifier is not valid");

            tenant = new Tenant(entity.TenantId, entity.Identifier);
            tenant.Categories=entity.Categories;
            tenant.ProductOrders=entity.ProductOrders;
            tenant.Products=entity.Products;
            tenant.Orders=entity.Orders;
